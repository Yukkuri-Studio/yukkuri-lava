...
class Lyrics extends Command {
  ...
  async run(i) {
    const song = i.options.getString("song")
      ...
      if (!player) return i.reply("No song playing right now")
      
      if (!player.currentTrack) return i.reply("No song playing right now")
      
    return this.lyricsSender(encodeURI(player.currentTrack.info.title), i)
    }
    
    return thislyricsSender(song, i)
  }
