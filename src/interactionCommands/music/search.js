...
class Search extends Command {
    ...
    async run(i) {
        ...
        const list = video.items.slice(0, 10).map((x, index) => `${++index} - ${x.title}`)
    ...
    async awaitInteraction(msg, member, player) {
      const i = await msg.channel.awaitMessageComponent({ filter: (i) => i.member.user.id === member.id, max: 1, errors: ["time"]});
      
      msg.delete().catch(er => er);
      const val = i.values[0]
      
      const resolve = await this.client.music.poru.resolve(`https://youtube.com/watch?v=${val}`);
